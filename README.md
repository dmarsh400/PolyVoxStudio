<div align="center"><div align="center"><div align="center"># PolyVox Studio

  

# 🎭 PolyVox Studio  



### *Many voices, one story.*# 🎭 PolyVox Studio  



[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)

[![Python 3.9+](https://img.shields.io/badge/python-3.9+-blue.svg)](https://www.python.org/downloads/)

[![GPU Support](https://img.shields.io/badge/GPU-CUDA%20%7C%20Legacy-green.svg)](GPU_COMPATIBILITY.md)### *Many voices, one story.*# 🎭 PolyVox Studio**Professional Audiobook Generation with AI Voice Synthesis**



**Transform your novels into immersive audiobooks with AI-powered character voice cloning**



[Features](#-features) • [Installation](#-installation) • [Quick Start](#-quick-start-guide) • [GPU Support](#-gpu-support) • [Documentation](#-documentation)[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)



![PolyVox Studio Interface](docs/screenshots/main_interface.png)[![Python 3.9+](https://img.shields.io/badge/python-3.9+-blue.svg)](https://www.python.org/downloads/)



</div>[![GPU Support](https://img.shields.io/badge/GPU-CUDA%20%7C%20Legacy-green.svg)](GPU_COMPATIBILITY.md)### *Many voices, one story.**Many voices, one story.*



---



## 🎯 What is PolyVox Studio?**Transform your novels into immersive audiobooks with AI-powered character voice cloning**



PolyVox Studio is a powerful desktop application that automatically converts novels and books into professional-quality audiobooks with **distinct AI voices for each character**. Using advanced NLP and voice cloning technology, it:



- 📖 **Automatically detects characters** in your book using BookNLP[Features](#-features) • [Installation](#-installation) • [Quick Start](#-quick-start) • [GPU Support](#-gpu-support) • [Documentation](#-documentation)[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](https://opensource.org/licenses/MIT)---

- 🗣️ **Assigns unique voices** to each character (or clone your own!)

- 🎙️ **Generates natural audiobook narration** with character dialogue

- ✏️ **Provides full editing control** over character assignments and dialogue

- 🎨 **Offers an intuitive GUI** built with CustomTkinter![PolyVox Studio Interface](docs/screenshots/main_interface.png)[![Python 3.9+](https://img.shields.io/badge/python-3.9+-blue.svg)](https://www.python.org/downloads/)



Whether you're an author wanting to hear your characters come alive, a reader who loves audiobooks, or a content creator producing audio content, PolyVox Studio makes it easy.



---</div>[![GPU Support](https://img.shields.io/badge/GPU-CUDA%20%7C%20Legacy-green.svg)](GPU_COMPATIBILITY.md)## 🎙️ Overview



## ✨ Features



### 🤖 Intelligent Character Detection---

- **Automatic character identification** using state-of-the-art BookNLP

- **Quote attribution** with speaker detection

- **Coreference resolution** to track character mentions

- **Handles complex narratives** with multiple POVs## 🎯 What is PolyVox Studio?**Transform your novels into immersive audiobooks with AI-powered character voice cloning**PolyVox Studio is a powerful desktop application for creating professional audiobooks with AI-generated voices. It features:



### 🎤 Advanced Voice Cloning

- **30+ built-in voices** across multiple accents and styles

- **Clone custom voices** from audio samples (5-15 seconds)PolyVox Studio is a powerful desktop application that automatically converts novels and books into professional-quality audiobooks with **distinct AI voices for each character**. Using advanced NLP and voice cloning technology, it:

- **Voice assignment UI** with visual character management

- **Narrator voice** for non-dialogue text



### 📚 Smart Text Processing- 📖 **Automatically detects characters** in your book using BookNLP[Features](#-features) • [Installation](#-installation) • [Quick Start](#-quick-start) • [GPU Support](#-gpu-support) • [Documentation](#-documentation)- **Intelligent Character Detection** - Automatically identifies characters and dialogue

- **Chapter detection** and segmentation

- **Dialogue extraction** with attribution confidence scoring- 🗣️ **Assigns unique voices** to each character (or clone your own!)

- **Handles various book formats** (TXT, EPUB coming soon)

- **BookNLP integration** for literary analysis- 🎙️ **Generates natural audiobook narration** with character dialogue- **Multi-Voice Support** - Assign different voices to each character and narrator



### 🎛️ Full Editorial Control- ✏️ **Provides full editing control** over character assignments and dialogue

- **Character merging** for variations (e.g., "John" + "Mr. Smith")

- **Character renaming** to fix detection errors- 🎨 **Offers an intuitive GUI** built with CustomTkinter![PolyVox Studio Interface](docs/screenshots/main_interface.png)- **XTTS v2 Integration** - High-quality voice cloning and synthesis

- **Line-by-line editing** of speaker assignments

- **Manual voice assignment** override



### ⚡ Performance & CompatibilityWhether you're an author wanting to hear your characters come alive, a reader who loves audiobooks, or a content creator producing audio content, PolyVox Studio makes it easy.- **Professional Audio Processing** - Automatic enhancement and normalization

- **GPU acceleration** (NVIDIA CUDA)

- **Legacy GPU support** for older cards (GTX 700+, K80)

- **CPU fallback mode** for systems without GPU

- **Batch processing** for long books---</div>- **Multi-GPU Support** - Efficient processing with automatic GPU load balancing



### 🎨 Modern Interface

- **Dark theme** CustomTkinter UI

- **Real-time progress tracking** with detailed logs## ✨ Features- **Chapter Management** - Smart chapter detection and organization

- **Debug console** for troubleshooting

- **Settings management** with persistent configuration



---### 🤖 Intelligent Character Detection---- **Easy Voice Cloning** - Clone voices from audio samples



## 🚀 Installation- **Automatic character identification** using state-of-the-art BookNLP



PolyVox Studio offers two installation paths depending on your GPU:- **Quote attribution** with speaker detection



### 🔥 Standard Installation- **Coreference resolution** to track character mentions

**For modern GPUs (RTX 20xx+, GTX 16xx)**

- **Handles complex narratives** with multiple POVs## 🎯 What is PolyVox Studio?---

```bash

# Clone the repository

git clone https://github.com/dmarsh400/PolyVoxStudio.git

cd PolyVoxStudio### 🎤 Advanced Voice Cloning



# Linux/Mac- **30+ built-in voices** across multiple accents and styles

./install.sh

- **Clone custom voices** from audio samples (5-15 seconds)PolyVox Studio is a powerful desktop application that automatically converts novels and books into professional-quality audiobooks with **distinct AI voices for each character**. Using advanced NLP and voice cloning technology, it:## 🚀 Quick Start

# Windows

install.bat- **Voice assignment UI** with visual character management

```

- **Narrator voice** for non-dialogue text

**Requirements:**

- Python 3.9 or higher

- NVIDIA GPU with Compute Capability 6.1+

- CUDA 12.1+ drivers### 📚 Smart Text Processing- 📖 **Automatically detects characters** in your book using BookNLP### Prerequisites

- 8GB+ VRAM recommended

- **Chapter detection** and segmentation

### 🐢 Legacy GPU Installation

**For older GPUs (GTX 700-1080, K80)**- **Dialogue extraction** with attribution confidence scoring- 🗣️ **Assigns unique voices** to each character (or clone your own!)



```bash- **Handles various book formats** (TXT, EPUB coming soon)

# Clone the repository

git clone https://github.com/dmarsh400/PolyVoxStudio.git- **BookNLP integration** for literary analysis- 🎙️ **Generates natural audiobook narration** with character dialogue- **Python 3.8+** (3.10 recommended)

cd PolyVoxStudio



# Linux/Mac

./install_legacy_gpu.sh### 🎛️ Full Editorial Control- ✏️ **Provides full editing control** over character assignments and dialogue- **8GB+ RAM** (16GB+ recommended)



# Windows- **Character merging** for variations (e.g., "John" + "Mr. Smith")

install_legacy_gpu.bat

```- **Character renaming** to fix detection errors- 🎨 **Offers an intuitive GUI** built with CustomTkinter- **Optional**: NVIDIA GPU with CUDA support for faster processing



**Requirements:**- **Line-by-line editing** of speaker assignments

- Python 3.9

- NVIDIA GPU with Compute Capability 3.5+- **Manual voice assignment** override

- CUDA 11.6 compatible drivers (450.80.02+)

- 6GB+ VRAM recommended



👉 **Not sure which installation to use?** Check the [GPU Compatibility Guide](GPU_COMPATIBILITY.md)### ⚡ Performance & CompatibilityWhether you're an author wanting to hear your characters come alive, a reader who loves audiobooks, or a content creator producing audio content, PolyVox Studio makes it easy.### Installation



### 📦 Dependencies- **GPU acceleration** (NVIDIA CUDA)



Both installations automatically set up:- **Legacy GPU support** for older cards (GTX 700+, K80)

- **PyTorch** (2.1+ or 1.13 for legacy)

- **Coqui TTS** for voice synthesis- **CPU fallback mode** for systems without GPU

- **BookNLP** for character detection

- **SpaCy** for NLP processing- **Batch processing** for long books---1. **Clone or download this repository**

- **CustomTkinter** for the GUI

- And many more...



---### 🎨 Modern Interface   ```bash



## 🎬 Quick Start Guide- **Dark theme** CustomTkinter UI



### 1️⃣ Launch the Application- **Real-time progress tracking** with detailed logs## ✨ Features   git clone https://github.com/yourusername/polyvox-studio.git



```bash- **Debug console** for troubleshooting

# Standard installation

./run_gui.sh          # Linux/Mac- **Settings management** with persistent configuration   cd polyvox-studio

run_gui.bat           # Windows



# Legacy GPU installation

./run_gui_legacy.sh   # Linux/Mac---### 🤖 Intelligent Character Detection   ```

run_gui_legacy.bat    # Windows

```



The application will open with a modern dark-themed interface.## 🚀 Installation- **Automatic character identification** using state-of-the-art BookNLP



---



### 2️⃣ Create Your First Audiobook (5 Easy Steps)PolyVox Studio offers two installation paths depending on your GPU:- **Quote attribution** with speaker detection2. **Run the installer**



#### **Step 1: Load Your Book** 📚

1. Click the **"Load Book"** button in the main interface

2. Select your text file (`.txt` format recommended)### 🔥 Standard Installation- **Coreference resolution** to track character mentions   ```bash

3. Fill in the book metadata:

   - **Title:** Your book's title**For modern GPUs (RTX 20xx+, GTX 16xx)**

   - **Author:** Author name  

   - **Genre:** Fiction, Mystery, Romance, etc.- **Handles complex narratives** with multiple POVs   python install.py

4. Click **"Confirm"** to load the book

```bash

> 💡 **Tip:** For best results, use plain text files with clear chapter breaks and properly formatted dialogue using standard quotation marks.

# Clone the repository   ```

#### **Step 2: Detect Characters** 🔍

1. Click the **"Run BookNLP & Attribution"** buttongit clone https://github.com/dmarsh400/PolyVoxStudio.git

2. Wait for processing (typically 1-5 minutes depending on book length and GPU)

3. The system will automatically:cd PolyVoxStudio### 🎤 Advanced Voice Cloning   

   - Identify all characters in your book

   - Extract dialogue and attribute it to speakers

   - Analyze narrative structure

   - Create character profiles# Linux/Mac- **30+ built-in voices** across multiple accents and styles   The installer will:



> 📊 **Progress:** Watch the status bar and console output for real-time processing updates../install.sh



#### **Step 3: Review & Refine Characters** ✏️- **Clone custom voices** from audio samples (5-15 seconds)   - Install all dependencies

1. Navigate to the **"Characters"** tab

2. Review the automatically detected characters# Windows

3. **Optional refinements:**

   - **Merge duplicates:** Select character variations (e.g., "John", "John Smith", "Mr. Smith") → Click **"Merge Characters"**install.bat- **Voice assignment UI** with visual character management   - Download required language models

   - **Rename characters:** Right-click any character → **"Rename"** to fix detection errors

   - **Remove minor characters:** Select characters → Click **"Remove"** to simplify```

4. Click on any character to preview their dialogue lines

- **Narrator voice** for non-dialogue text   - Optionally create a desktop icon

> ⚠️ **Common Issue:** Character variations detected separately? Just select them all and click "Merge Characters"!

**Requirements:**

#### **Step 4: Assign Voices** 🎤

1. Navigate to the **"Clone Voices"** tab- Python 3.9 or higher

2. For each main character, choose from two options:

- NVIDIA GPU with Compute Capability 6.1+

   **Option A: Use Built-in Voices** (Fastest)

   - Browse 30+ professionally crafted voices- CUDA 12.1+ drivers### 📚 Smart Text Processing3. **Launch PolyVox Studio**

   - Click the play icon to preview each voice

   - Drag voice onto character OR select both and click **"Assign"**- 8GB+ VRAM recommended



   **Option B: Clone a Custom Voice** (Most Unique)- **Chapter detection** and segmentation   

   - Click **"Upload Audio Sample"**

   - Select a 5-15 second clean audio file (WAV/MP3)### 🐢 Legacy GPU Installation

   - Enter voice details (name, gender, accent)

   - Click **"Process Voice"** and wait for cloning**For older GPUs (GTX 700-1080, K80)**- **Dialogue extraction** with attribution confidence scoring   **Linux/Mac:**

   - Assign the new voice to your character



3. **Assign Narrator Voice:**

   - Select a neutral, clear voice for non-dialogue narration```bash- **Handles various book formats** (TXT, EPUB coming soon)   ```bash

   - Assign using the **"Narrator Voice"** dropdown

# Clone the repository

> 🎯 **Best Practice:** Use distinct, contrasting voices for main characters who interact frequently. Similar voices can confuse listeners!

git clone https://github.com/dmarsh400/PolyVoxStudio.git- **BookNLP integration** for literary analysis   ./run_gui.sh

#### **Step 5: Generate Your Audiobook** 🎧

1. Navigate to the **"Settings"** tabcd PolyVoxStudio

2. Configure output settings:

   - **Output Directory:** Where to save audio files   ```

   - **Audio Format:** WAV (highest quality) or MP3 (smaller file size)

   - **Sample Rate:** 22050 Hz (standard) or 44100 Hz (higher quality)# Linux/Mac

3. Click **"Generate Audiobook"** button

4. Wait for processing (varies by length, typically 5-30 minutes for an average novel)./install_legacy_gpu.sh### 🎛️ Full Editorial Control   

5. Find your completed audiobook files in the output directory!



> ⏱️ **Estimated Processing Times:** 

> - Short story (5,000 words): ~5 minutes# Windows- **Character merging** for variations (e.g., "John" + "Mr. Smith")   **Windows:**

> - Standard novel (80,000 words): ~20-30 minutes

> - Long novel (150,000+ words): ~45-60 minutesinstall_legacy_gpu.bat



---```- **Character renaming** to fix detection errors   ```bash



### 3️⃣ Advanced Workflows



#### 🔄 Merge Character Variations**Requirements:**- **Line-by-line editing** of speaker assignments   run_gui.bat

When characters like "John", "John Smith", and "Mr. Smith" are detected separately:

1. Go to **Characters tab**- Python 3.9

2. Hold **Ctrl** (Windows/Linux) or **Cmd** (Mac) and click each variation

3. Click the **"Merge Characters"** button- NVIDIA GPU with Compute Capability 3.5+- **Manual voice assignment** override   ```

4. Select which name to use as the primary identifier

5. Confirm the merge- CUDA 11.6 compatible drivers (450.80.02+)



**Result:** All dialogue consolidates under one character with one consistent voice.- 6GB+ VRAM recommended   



#### 🎙️ Clone Custom Voices

Create truly unique character voices from audio samples:

1. Go to **Clone Voices tab**👉 **Not sure which installation to use?** Check the [GPU Compatibility Guide](GPU_COMPATIBILITY.md)### ⚡ Performance & Compatibility   Or use the desktop icon if you created one.

2. Click **"Upload Audio"** button

3. Select your audio file (requirements):

   - **Duration:** 5-15 seconds (10+ seconds ideal)

   - **Quality:** Clear speech, minimal background noise### 📦 Dependencies- **GPU acceleration** (NVIDIA CUDA)

   - **Format:** WAV, MP3, or FLAC

   - **Content:** Single speaker, natural conversational speech

4. Fill in voice metadata:

   - **Name:** Descriptive name (e.g., "Elderly British Woman")Both installations automatically set up:- **Legacy GPU support** for older cards (GTX 700+, K80)---

   - **Gender:** Male/Female/Other

   - **Accent/Style:** American, British, Australian, etc.- **PyTorch** (2.1+ or 1.13 for legacy)

5. Click **"Add Voice"** to process

6. Once processed, assign to characters- **Coqui TTS** for voice synthesis- **CPU fallback mode** for systems without GPU



> 🎬 **Pro Tip:** Use professional voice actor samples, audiobook clips, or high-quality podcast audio for best cloning results!- **BookNLP** for character detection



#### ✏️ Edit Dialogue Attribution- **SpaCy** for NLP processing- **Batch processing** for long books## 📚 Features

Manually fix incorrect speaker assignments:

1. Select a character in the **Characters tab**- **CustomTkinter** for the GUI

2. Browse their assigned dialogue lines in the preview pane

3. To reassign a line:- And many more...

   - Click the line to select it

   - Click the **"Change Speaker"** dropdown

   - Select the correct character name

   - Confirm the change---### 🎨 Modern Interface### 1. Book Processing

4. Changes save automatically to the project



**Common fixes needed:**

- Narrator lines mistakenly attributed to characters## 🎬 Quick Start- **Dark theme** CustomTkinter UI- Load `.txt`, `.epub`, or `.pdf` files

- Character dialogue incorrectly marked as narration

- Dialogue assigned to the wrong character



#### 📖 Chapter-by-Chapter Processing### 1️⃣ Launch the Application- **Real-time progress tracking** with detailed logs- Automatic chapter detection

For very long books or testing workflows:

1. Use the **"Load Specific Chapters"** option when loading books

2. Enter chapter range (e.g., "1-3" for first three chapters, or "5" for chapter 5 only)

3. Process and review smaller sections independently```bash- **Debug console** for troubleshooting- Smart text segmentation

4. Adjust settings and workflow as needed

5. Process full book once satisfied# Standard installation



#### 🔊 Batch Voice Assignment./run_gui.sh          # Linux/Mac- **Settings management** with persistent configuration

Quickly assign voices to multiple characters:

1. In **Clone Voices tab**, hold **Ctrl/Cmd** and select multiple charactersrun_gui.bat           # Windows

2. Choose a voice from the voice library

3. Click **"Assign to Selected"** button### 2. Character Detection

4. All selected characters receive the same voice

# Legacy GPU installation

> ⚠️ **Note:** Only recommended for very minor background characters. Main characters should always have distinct, unique voices!

./run_gui_legacy.sh   # Linux/Mac---- AI-powered character identification

---

run_gui_legacy.bat    # Windows

### 4️⃣ Troubleshooting Quick Fixes

```- Dialogue attribution

| Problem | Solution |

|---------|----------|

| **"No characters detected"** | Ensure your text has dialogue with quotation marks. Check text formatting. |

| **"GPU out of memory"** | Reduce batch size: Settings → Advanced → Batch Size (try 4 or 2) |### 2️⃣ Process Your First Book## 🚀 Installation- Quote extraction

| **"Poor character detection"** | Preprocess text: ensure clear chapter breaks and consistent dialogue formatting |

| **"Voice sounds robotic"** | Use longer, higher quality audio samples for cloning (10-15 seconds minimum) |

| **"Processing very slow"** | Verify GPU usage: Settings → Debug → Show GPU Info. Check GPU drivers. |

| **"Application won't start"** | Check Python version (3.9+ required). Reinstall dependencies. |1. **Click "Load Book"** and select your text file- Speaker recognition

| **"Audio cuts off mid-sentence"** | Increase audio buffer in Settings → Advanced → Audio Buffer Size |

2. **Enter book details** (title, author, genre)

📖 **Need more help?** Check the full [FAQ](docs/FAQ.md) or [Troubleshooting Guide](docs/TROUBLESHOOTING.md)

3. **Click "Run BookNLP & Attribution"** to detect charactersPolyVox Studio offers two installation paths depending on your GPU:

---

4. **Review character detection** in the Characters tab

### 5️⃣ Tips for Professional Results

5. **Assign voices** in the Clone Voices tab### 3. Voice Management

✅ **Text Preparation:**

- Use standard quotation marks (" ") for dialogue6. **Generate audio** in the Settings tab

- Include clear chapter breaks and section dividers

- Remove excessive whitespace, page numbers, or formatting artifacts<table>- Extensive voice library

- Ensure consistent punctuation throughout

### 3️⃣ Advanced Workflows

✅ **Character Detection:**

- Books with 5-15 main characters work best<tr>- Voice cloning from audio samples

- Very large casts (50+ characters) may require manual cleanup

- First-person narratives require special narrator handling#### Merge Character Variations

- Name consistency in text improves detection accuracy

If "John", "John Smith", and "Mr. Smith" are detected as separate characters:<td width="50%">- Per-character voice assignment

✅ **Voice Selection Strategy:**

- Match voice characteristics (age, gender, accent) to character descriptions1. Go to **Characters tab**

- Use high-contrast voices for frequently interacting characters

- Reserve the clearest, most neutral voice for the narrator2. Select all variations- Preview voices before processing

- Test voice combinations before full audiobook generation

3. Click **"Merge Characters"**

✅ **Performance Optimization:**

- Close other GPU-intensive applications during processing4. Choose which name to keep### 🔥 Standard Installation

- Process shorter books first to optimize your workflow

- Save projects regularly: Settings → Save Project

- Use SSD storage for faster file I/O

#### Clone Custom Voices**For modern GPUs (RTX 20xx+, GTX 16xx)**### 4. Audio Generation

---

1. Go to **Clone Voices tab**

### 6️⃣ Example Workflow: "The Great Gatsby"

2. Click **"Upload Audio"** (5-15 second sample)- High-quality XTTS v2 synthesis

Here's a complete real-world example:

3. Enter voice metadata

**Step 1: Load Book**

- File: `the_great_gatsby.txt`4. Assign to characters```bash- Multi-voice support

- Title: "The Great Gatsby"

- Author: "F. Scott Fitzgerald"

- Genre: "Literary Fiction"

#### Edit Dialogue Attribution# Clone the repository- Automatic audio enhancement

**Step 2: BookNLP Detection**

- Automatically detects: Nick, Gatsby, Daisy, Tom, Jordan, Wilson, Myrtle1. Select a character in **Characters tab**

- Processes 47,094 words in ~2 minutes

2. View all their dialogue linesgit clone https://github.com/dmarsh400/PolyVoxStudio.git- Progress tracking

**Step 3: Character Cleanup**

- Merge "Jay Gatsby" + "Gatsby" + "Mr. Gatsby" → "Gatsby"3. Click any line to reassign speaker

- Merge "Tom Buchanan" + "Tom" → "Tom"

- Merge "Daisy Buchanan" + "Daisy" → "Daisy"4. Changes save automaticallycd PolyVoxStudio- Chapter-by-chapter processing

- Remove minor characters: Owl Eyes, Klipspringer



**Step 4: Voice Assignment**

- **Nick Carraway (Narrator):** Neutral, contemplative male voice (built-in: "thoughtful_male")---

- **Gatsby:** Smooth, charismatic voice with slight mystique (built-in: "charming_gentleman")

- **Daisy:** Young, delicate, upper-class female voice (built-in: "southern_belle")

- **Tom:** Aggressive, booming, arrogant male voice (built-in: "dominant_male")

- **Jordan:** Cool, sophisticated, aloof female voice (built-in: "sophisticated_woman")## 🖥️ GPU Support# Linux/Mac### 5. GPU Acceleration



**Step 5: Generate**

- Format: MP3 (44.1kHz)

- Processing time: ~18 minutes on RTX 3080PolyVox Studio leverages GPU acceleration for significantly faster processing:./install.sh- Automatic GPU detection

- Output: 6 chapter files + full audiobook (4 hours 2 minutes)



🎧 **Result:** A professionally voiced audiobook that brings the Jazz Age to vivid life with distinct character personalities!

| Task | RTX 3080 | GTX 1080 | K80 | CPU Only |- Multi-GPU load balancing

---

|------|----------|----------|-----|----------|

## 🖥️ GPU Support

| **Character Detection** | 30-60s | 1-2 min | 2-5 min | 5-10 min |# Windows- CPU fallback support

PolyVox Studio leverages GPU acceleration for significantly faster processing:

| **Audio Generation** | 15-30s/min | 30-60s/min | 1-2 min/min | 5-10 min/min |

| Task | RTX 3080 | GTX 1080 | K80 | CPU Only |

|------|----------|----------|-----|----------|install.bat- Real-time GPU monitoring

| **Character Detection** | 30-60s | 1-2 min | 2-5 min | 5-10 min |

| **Audio Generation** | 15-30s/min | 30-60s/min | 1-2 min/min | 5-10 min/min |### Supported GPUs



### Supported GPUs```



✅ **Standard Installation:**✅ **Standard Installation:**

- RTX 40 series (4090, 4080, 4070)

- RTX 30 series (3090, 3080, 3070, 3060)- RTX 40 series (4090, 4080, 4070)---

- RTX 20 series (2080 Ti, 2070, 2060)

- GTX 16 series (1660 Ti, 1650)- RTX 30 series (3090, 3080, 3070, 3060)



✅ **Legacy Installation:**- RTX 20 series (2080 Ti, 2070, 2060)**Requirements:**

- GTX 10 series (1080 Ti, 1070, 1060)

- GTX 900 series (980 Ti, 970, 960)- GTX 16 series (1660 Ti, 1650)

- GTX 700 series (780 Ti, 770, 760)

- Tesla K80, K40- Python 3.9 or higher## 🎨 Usage

- Quadro K-series

✅ **Legacy Installation:**

📖 **Full compatibility guide:** [GPU_COMPATIBILITY.md](GPU_COMPATIBILITY.md)

- GTX 10 series (1080 Ti, 1070, 1060)- NVIDIA GPU with Compute Capability 6.1+

---

- GTX 900 series (980 Ti, 970, 960)

## 📖 Documentation

- GTX 700 series (780 Ti, 770, 760)- CUDA 12.1+ drivers### Basic Workflow

### Core Guides

- [Installation Guide](docs/INSTALLATION.md) - Detailed setup instructions- Tesla K80, K40

- [User Guide](docs/USER_GUIDE.md) - Step-by-step usage tutorial

- [GPU Compatibility](GPU_COMPATIBILITY.md) - Which installation for your GPU- Quadro K-series- 8GB+ VRAM recommended

- [Legacy GPU Setup](LEGACY_GPU_INSTALL.md) - Older GPU installation details



### Technical Documentation

- [Architecture Overview](docs/ARCHITECTURE.md) - How PolyVox works📖 **Full compatibility guide:** [GPU_COMPATIBILITY.md](GPU_COMPATIBILITY.md)1. **Load Your Book**

- [Character Detection](docs/CHARACTER_DETECTION.md) - BookNLP integration details

- [Voice Cloning](docs/VOICE_CLONING.md) - TTS and voice synthesis

- [API Reference](docs/API.md) - For developers and contributors

---</td>   - Go to the "Book Processing" tab

### Troubleshooting

- [FAQ](docs/FAQ.md) - Common questions and answers

- [Troubleshooting Guide](docs/TROUBLESHOOTING.md) - Fix common issues

- [Performance Tuning](docs/PERFORMANCE.md) - Optimize for your system## 📖 Documentation<td width="50%">   - Click "Load Book" and select your file



---



## 🛠️ System Requirements### Core Guides   - Review detected chapters



### Minimum Requirements- [Installation Guide](docs/INSTALLATION.md) - Detailed setup instructions

- **OS:** Windows 10+, Linux (Ubuntu 20.04+), macOS 11+

- **CPU:** 4-core processor (Intel i5 / AMD Ryzen 5 or better)- [User Guide](docs/USER_GUIDE.md) - Step-by-step usage tutorial### 🐢 Legacy GPU Installation

- **RAM:** 8GB (16GB recommended)

- **Storage:** 10GB free space (models + workspace)- [GPU Compatibility](GPU_COMPATIBILITY.md) - Which installation for your GPU

- **GPU:** NVIDIA GPU with 6GB VRAM (optional but recommended)

- [Legacy GPU Setup](LEGACY_GPU_INSTALL.md) - Older GPU installation details**For older GPUs (GTX 700-1080, K80)**2. **Detect Characters**

### Recommended for Best Experience

- **CPU:** 8-core processor (Intel i7/i9 or AMD Ryzen 7/9)

- **RAM:** 16GB or more

- **GPU:** RTX 3060 or better with 12GB+ VRAM### Technical Documentation   - Switch to the "Characters" tab

- **Storage:** SSD with 20GB+ free space

- [Architecture Overview](docs/ARCHITECTURE.md) - How PolyVox works

---

- [Character Detection](docs/CHARACTER_DETECTION.md) - BookNLP integration details```bash   - Click "Detect Characters"

## 🎨 Screenshots

- [Voice Cloning](docs/VOICE_CLONING.md) - TTS and voice synthesis

<div align="center">

- [API Reference](docs/API.md) - For developers and contributors# Clone the repository   - Review identified characters and dialogue

### Main Interface

![Main Interface](docs/screenshots/main_interface.png)



### Character Detection### Troubleshootinggit clone https://github.com/dmarsh400/PolyVoxStudio.git

![Character Detection](docs/screenshots/characters_tab.png)

- [FAQ](docs/FAQ.md) - Common questions and answers

### Voice Assignment

![Voice Cloning](docs/screenshots/clone_voices_tab.png)- [Troubleshooting Guide](docs/TROUBLESHOOTING.md) - Fix common issuescd PolyVoxStudio3. **Assign Voices**



### Settings & Configuration- [Performance Tuning](docs/PERFORMANCE.md) - Optimize for your system

![Settings](docs/screenshots/settings_tab.png)

   - Go to the "Voices" tab

</div>

---

---

# Linux/Mac   - Assign a voice to each character

## 🤝 Contributing

## 🛠️ System Requirements

We welcome contributions! Whether you're fixing bugs, adding features, or improving documentation, your help makes PolyVox better.

./install_legacy_gpu.sh   - Preview voices before finalizing

### How to Contribute

1. Fork the repository### Minimum Requirements

2. Create a feature branch (`git checkout -b feature/AmazingFeature`)

3. Commit your changes (`git commit -m 'Add some AmazingFeature'`)- **OS:** Windows 10+, Linux (Ubuntu 20.04+), macOS 11+

4. Push to the branch (`git push origin feature/AmazingFeature`)

5. Open a Pull Request- **CPU:** 4-core processor (Intel i5 / AMD Ryzen 5 or better)



See [CONTRIBUTING.md](CONTRIBUTING.md) for detailed guidelines.- **RAM:** 8GB (16GB recommended)# Windows4. **Generate Audio**



### Development Setup- **Storage:** 10GB free space (models + workspace)

```bash

# Clone your fork- **GPU:** NVIDIA GPU with 6GB VRAM (optional but recommended)install_legacy_gpu.bat   - Switch to "Audio Processing" tab

git clone https://github.com/YOUR_USERNAME/PolyVoxStudio.git

cd PolyVoxStudio



# Install in development mode### Recommended for Best Experience```   - Click "Generate Audio"

pip install -e .

- **CPU:** 8-core processor (Intel i7/i9 or AMD Ryzen 7/9)

# Run tests

python -m pytest tests/- **RAM:** 16GB or more   - Wait for processing to complete

```

- **GPU:** RTX 3060 or better with 12GB+ VRAM

---

- **Storage:** SSD with 20GB+ free space**Requirements:**   - Find your audiobook in the `output_audio/` folder

## 📜 License



This project is licensed under the MIT License - see the [LICENSE](LICENSE) file for details.

---- Python 3.9

### Third-Party Licenses

- **BookNLP** - Apache 2.0 License

- **Coqui TTS** - Mozilla Public License 2.0

- **PyTorch** - BSD-style License## 🎨 Screenshots- NVIDIA GPU with Compute Capability 3.5+### Advanced Features

- **SpaCy** - MIT License



---

<div align="center">- CUDA 11.6 compatible drivers (450.80.02+)

## 🙏 Acknowledgments



PolyVox Studio is built on the shoulders of giants:

### Main Interface- 6GB+ VRAM recommended#### Voice Cloning

- **[BookNLP](https://github.com/booknlp/booknlp)** by David Bamman - Character detection and literary NLP

- **[Coqui TTS](https://github.com/coqui-ai/TTS)** - High-quality voice synthesis![Main Interface](docs/screenshots/main_interface.png)

- **[XTTS v2](https://huggingface.co/coqui/XTTS-v2)** - Multilingual voice cloning

- **[PyTorch](https://pytorch.org/)** - Deep learning framework1. Go to "Clone Voices" tab

- **[CustomTkinter](https://github.com/TomSchimansky/CustomTkinter)** - Modern GUI framework

### Character Detection

Special thanks to all contributors and the open-source community!

![Character Detection](docs/screenshots/characters_tab.png)</td>2. Record or upload a 10-30 second audio sample

---



## 📞 Support

### Voice Assignment</tr>3. Enter voice details

### Get Help

- 📖 Check the [Documentation](docs/)![Voice Cloning](docs/screenshots/clone_voices_tab.png)

- ❓ Read the [FAQ](docs/FAQ.md)

- 🐛 Report bugs via [Issues](https://github.com/dmarsh400/PolyVoxStudio/issues)</table>4. Click "Save Voice"

- 💬 Join discussions in [Discussions](https://github.com/dmarsh400/PolyVoxStudio/discussions)

### Settings & Configuration

### Stay Updated

- ⭐ Star this repository to stay notified![Settings](docs/screenshots/settings_tab.png)5. Use the cloned voice like any other voice

- 👁️ Watch for updates and releases

- 🍴 Fork to create your own version



---</div>👉 **Not sure which installation to use?** Check the [GPU Compatibility Guide](GPU_COMPATIBILITY.md)



## 🗺️ Roadmap



### Coming Soon---#### Chapter-by-Chapter Processing

- [ ] EPUB file support

- [ ] Audio export formats (M4B, OGG)

- [ ] Batch processing multiple books

- [ ] Cloud rendering options## 🤝 Contributing### 📦 Dependencies- Process large books one chapter at a time

- [ ] Mobile companion app



### Future Enhancements

- [ ] Emotional tone detection for voice modulationWe welcome contributions! Whether you're fixing bugs, adding features, or improving documentation, your help makes PolyVox better.- Each chapter is processed independently

- [ ] Multi-language support

- [ ] Advanced audio post-processing

- [ ] Character voice training from book descriptions

- [ ] API for programmatic access### How to ContributeBoth installations automatically set up:- No cross-contamination between chapters



---1. Fork the repository



<div align="center">2. Create a feature branch (`git checkout -b feature/AmazingFeature`)- **PyTorch** (2.1+ or 1.13 for legacy)



**Made with ❤️ by the PolyVox team**3. Commit your changes (`git commit -m 'Add some AmazingFeature'`)



[⬆ Back to Top](#-polyvox-studio)4. Push to the branch (`git push origin feature/AmazingFeature`)- **Coqui TTS** for voice synthesis---



</div>5. Open a Pull Request


- **BookNLP** for character detection

See [CONTRIBUTING.md](CONTRIBUTING.md) for detailed guidelines.

- **SpaCy** for NLP processing## ⚙️ Configuration

### Development Setup

```bash- **CustomTkinter** for the GUI

# Clone your fork

git clone https://github.com/YOUR_USERNAME/PolyVoxStudio.git- And many more...### GPU Setup

cd PolyVoxStudio



# Install in development mode

pip install -e .---PolyVox Studio automatically detects and uses available GPUs. To force CPU mode:



# Run tests

python -m pytest tests/

```## 🎬 Quick StartEdit `run_gui.sh` (Linux/Mac) or `run_gui.bat` (Windows):



---```bash



## 📜 License### 1️⃣ Launch the Applicationexport CUDA_VISIBLE_DEVICES=""  # Force CPU mode



This project is licensed under the MIT License - see the [LICENSE](LICENSE) file for details.```



### Third-Party Licenses```bash

- **BookNLP** - Apache 2.0 License

- **Coqui TTS** - Mozilla Public License 2.0# Standard installation### Audio Quality

- **PyTorch** - BSD-style License

- **SpaCy** - MIT License./run_gui.sh          # Linux/Mac



---run_gui.bat           # WindowsAudio enhancement requires FFmpeg. Install it:



## 🙏 Acknowledgments



PolyVox Studio is built on the shoulders of giants:# Legacy GPU installation**Linux:**



- **[BookNLP](https://github.com/booknlp/booknlp)** by David Bamman - Character detection and literary NLP./run_gui_legacy.sh   # Linux/Mac```bash

- **[Coqui TTS](https://github.com/coqui-ai/TTS)** - High-quality voice synthesis

- **[XTTS v2](https://huggingface.co/coqui/XTTS-v2)** - Multilingual voice cloningrun_gui_legacy.bat    # Windowssudo apt install ffmpeg

- **[PyTorch](https://pytorch.org/)** - Deep learning framework

- **[CustomTkinter](https://github.com/TomSchimansky/CustomTkinter)** - Modern GUI framework``````



Special thanks to all contributors and the open-source community!



---### 2️⃣ Process Your First Book**Mac:**



## 📞 Support```bash



### Get Help1. **Click "Load Book"** and select your text filebrew install ffmpeg

- 📖 Check the [Documentation](docs/)

- ❓ Read the [FAQ](docs/FAQ.md)2. **Enter book details** (title, author, genre)```

- 🐛 Report bugs via [Issues](https://github.com/dmarsh400/PolyVoxStudio/issues)

- 💬 Join discussions in [Discussions](https://github.com/dmarsh400/PolyVoxStudio/discussions)3. **Click "Run BookNLP & Attribution"** to detect characters



### Stay Updated4. **Review character detection** in the Characters tab**Windows:**

- ⭐ Star this repository to stay notified

- 👁️ Watch for updates and releases5. **Assign voices** in the Clone Voices tabDownload from [ffmpeg.org](https://ffmpeg.org/download.html)

- 🍴 Fork to create your own version

6. **Generate audio** in the Settings tab

---

---

## 🗺️ Roadmap

### 3️⃣ Advanced Workflows

### Coming Soon

- [ ] EPUB file support## 📖 System Requirements

- [ ] Audio export formats (M4B, OGG)

- [ ] Batch processing multiple books#### Merge Character Variations

- [ ] Cloud rendering options

- [ ] Mobile companion appIf "John", "John Smith", and "Mr. Smith" are detected as separate characters:### Minimum



### Future Enhancements1. Go to **Characters tab**- Python 3.8+

- [ ] Emotional tone detection for voice modulation

- [ ] Multi-language support2. Select all variations- 8GB RAM

- [ ] Advanced audio post-processing

- [ ] Character voice training from book descriptions3. Click **"Merge Characters"**- 10GB free disk space

- [ ] API for programmatic access

4. Choose which name to keep- CPU: Multi-core processor

---



<div align="center">

#### Clone Custom Voices### Recommended

**Made with ❤️ by the PolyVox team**

1. Go to **Clone Voices tab**- Python 3.10+

[⬆ Back to Top](#-polyvox-studio)

2. Click **"Upload Audio"** (5-15 second sample)- 16GB+ RAM

</div>

3. Enter voice metadata- 20GB+ free disk space

4. Assign to characters- NVIDIA GPU with 8GB+ VRAM

- CUDA 11.8 or higher

#### Edit Dialogue Attribution

1. Select a character in **Characters tab**---

2. View all their dialogue lines

3. Click any line to reassign speaker## 🛠️ Troubleshooting

4. Changes save automatically

### Common Issues

---

**Issue:** "ModuleNotFoundError: No module named 'XXX'"

## 🖥️ GPU Support**Solution:** Run `python install.py` again or `pip install -r requirements_min.txt`



PolyVox Studio leverages GPU acceleration for significantly faster processing:**Issue:** Slow processing

**Solution:** Check GPU is being used. Install CUDA if you have NVIDIA GPU.

| Task | RTX 3080 | GTX 1080 | K80 | CPU Only |

|------|----------|----------|-----|----------|**Issue:** Audio quality is poor

| **Character Detection** | 30-60s | 1-2 min | 2-5 min | 5-10 min |**Solution:** Install FFmpeg for audio enhancement

| **Audio Generation** | 15-30s/min | 30-60s/min | 1-2 min/min | 5-10 min/min |

**Issue:** Chapter detection creates parts

### Supported GPUs**Solution:** Run `./clear_chapter_cache.sh` and re-detect chapters



✅ **Standard Installation:****Issue:** Characters not detected properly

- RTX 40 series (4090, 4080, 4070)**Solution:** Process chapters individually for better results

- RTX 30 series (3090, 3080, 3070, 3060)

- RTX 20 series (2080 Ti, 2070, 2060)---

- GTX 16 series (1660 Ti, 1650)

## 📝 File Structure

✅ **Legacy Installation:**

- GTX 10 series (1080 Ti, 1070, 1060)```

- GTX 900 series (980 Ti, 970, 960)polyvox-studio/

- GTX 700 series (780 Ti, 770, 760)├── app/                    # Main application code

- Tesla K80, K40│   ├── core/              # Core processing logic

- Quadro K-series│   ├── engine/            # TTS engines

│   └── ui/                # User interface

📖 **Full compatibility guide:** [GPU_COMPATIBILITY.md](GPU_COMPATIBILITY.md)├── assets/                # Application assets

│   └── polyvox_splash.png

---├── output/                # BookNLP processing output

├── output_audio/          # Generated audiobooks

## 📖 Documentation├── voices/                # Voice sample library

├── cleaned/               # Archive of dev files

### Core Guides├── requirements_min.txt   # Python dependencies

- [Installation Guide](docs/INSTALLATION.md) - Detailed setup instructions├── install.py             # Installation script

- [User Guide](docs/USER_GUIDE.md) - Step-by-step usage tutorial├── run_gui.sh             # Launch script (Linux/Mac)

- [GPU Compatibility](GPU_COMPATIBILITY.md) - Which installation for your GPU├── run_gui.bat            # Launch script (Windows)

- [Legacy GPU Setup](LEGACY_GPU_INSTALL.md) - Older GPU installation details└── USER_GUIDE.pdf         # Comprehensive user guide

```

### Technical Documentation

- [Architecture Overview](docs/ARCHITECTURE.md) - How PolyVox works---

- [Character Detection](docs/CHARACTER_DETECTION.md) - BookNLP integration details

- [Voice Cloning](docs/VOICE_CLONING.md) - TTS and voice synthesis## 🤝 Contributing

- [API Reference](docs/API.md) - For developers and contributors

Contributions are welcome! Please:

### Troubleshooting

- [FAQ](docs/FAQ.md) - Common questions and answers1. Fork the repository

- [Troubleshooting Guide](docs/TROUBLESHOOTING.md) - Fix common issues2. Create a feature branch

- [Performance Tuning](docs/PERFORMANCE.md) - Optimize for your system3. Make your changes

4. Submit a pull request

---

---

## 🛠️ System Requirements

## 📄 License

### Minimum Requirements

- **OS:** Windows 10+, Linux (Ubuntu 20.04+), macOS 11+[Choose your license - MIT, GPL, etc.]

- **CPU:** 4-core processor (Intel i5 / AMD Ryzen 5 or better)

- **RAM:** 8GB (16GB recommended)---

- **Storage:** 10GB free space (models + workspace)

- **GPU:** NVIDIA GPU with 6GB VRAM (optional but recommended)## 🙏 Acknowledgments



### Recommended for Best Experience- **Coqui TTS** - XTTS v2 voice synthesis

- **CPU:** 8-core processor (Intel i7/i9 or AMD Ryzen 7/9)- **BookNLP** - Character and dialogue detection

- **RAM:** 16GB or more- **CustomTkinter** - Modern GUI framework

- **GPU:** RTX 3060 or better with 12GB+ VRAM- **Transformers** - NLP models

- **Storage:** SSD with 20GB+ free space- All open-source contributors



------



## 🎨 Screenshots## 📧 Support



<div align="center">- **Issues**: [GitHub Issues](https://github.com/yourusername/polyvox-studio/issues)

- **Discussions**: [GitHub Discussions](https://github.com/yourusername/polyvox-studio/discussions)

### Main Interface- **Documentation**: See `USER_GUIDE.pdf`

![Main Interface](docs/screenshots/main_interface.png)

---

### Character Detection

![Character Detection](docs/screenshots/characters_tab.png)## 🎉 Features Coming Soon



### Voice Assignment- [ ] Azure voice synthesis support

![Voice Cloning](docs/screenshots/clone_voices_tab.png)- [ ] Real-time voice preview

- [ ] Batch processing multiple books

### Settings & Configuration- [ ] Audio editing tools

![Settings](docs/screenshots/settings_tab.png)- [ ] Export to various formats

- [ ] Cloud rendering support

</div>

---

---

**PolyVox Studio** - Turn your books into audiobooks with many voices, one story. 🎙️📚

## 🤝 Contributing

We welcome contributions! Whether you're fixing bugs, adding features, or improving documentation, your help makes PolyVox better.

### How to Contribute
1. Fork the repository
2. Create a feature branch (`git checkout -b feature/AmazingFeature`)
3. Commit your changes (`git commit -m 'Add some AmazingFeature'`)
4. Push to the branch (`git push origin feature/AmazingFeature`)
5. Open a Pull Request

See [CONTRIBUTING.md](CONTRIBUTING.md) for detailed guidelines.

### Development Setup
```bash
# Clone your fork
git clone https://github.com/YOUR_USERNAME/PolyVoxStudio.git
cd PolyVoxStudio

# Install in development mode
pip install -e .

# Run tests
python -m pytest tests/
```

---

## 📜 License

This project is licensed under the MIT License - see the [LICENSE](LICENSE) file for details.

### Third-Party Licenses
- **BookNLP** - Apache 2.0 License
- **Coqui TTS** - Mozilla Public License 2.0
- **PyTorch** - BSD-style License
- **SpaCy** - MIT License

---

## 🙏 Acknowledgments

PolyVox Studio is built on the shoulders of giants:

- **[BookNLP](https://github.com/booknlp/booknlp)** by David Bamman - Character detection and literary NLP
- **[Coqui TTS](https://github.com/coqui-ai/TTS)** - High-quality voice synthesis
- **[XTTS v2](https://huggingface.co/coqui/XTTS-v2)** - Multilingual voice cloning
- **[PyTorch](https://pytorch.org/)** - Deep learning framework
- **[CustomTkinter](https://github.com/TomSchimansky/CustomTkinter)** - Modern GUI framework

Special thanks to all contributors and the open-source community!

---

## 📞 Support

### Get Help
- 📖 Check the [Documentation](docs/)
- ❓ Read the [FAQ](docs/FAQ.md)
- 🐛 Report bugs via [Issues](https://github.com/dmarsh400/PolyVoxStudio/issues)
- 💬 Join discussions in [Discussions](https://github.com/dmarsh400/PolyVoxStudio/discussions)

### Stay Updated
- ⭐ Star this repository to stay notified
- 👁️ Watch for updates and releases
- 🍴 Fork to create your own version

---

## 🗺️ Roadmap

### Coming Soon
- [ ] EPUB file support
- [ ] Audio export formats (M4B, OGG)
- [ ] Batch processing multiple books
- [ ] Cloud rendering options
- [ ] Mobile companion app

### Future Enhancements
- [ ] Emotional tone detection for voice modulation
- [ ] Multi-language support
- [ ] Advanced audio post-processing
- [ ] Character voice training from book descriptions
- [ ] API for programmatic access

---

<div align="center">

**Made with ❤️ by the PolyVox team**

[⬆ Back to Top](#-polyvox-studio)

</div>
