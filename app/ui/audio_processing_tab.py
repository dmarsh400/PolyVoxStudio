...
class AudioProcessingTab(ctk.CTkFrame):
    ...
    def _split_long_text(self, text: str, max_chars: int = 200) -> list:
        """Split narration into sentence-aware chunks without adding ellipses."""
        text = (text or "").strip()
        if not text:
            return [text]
        ...
        if len(sentences) == 1:
            return self._force_chunk_by_words(normalized, max_chars)
        ...
        for sentence in sentences:
            ...
            if len(sentence) <= max_chars:
                ...
                forced = self._force_chunk_by_words(sentence, max_chars)
                if forced:
    ...
    def _force_chunk_by_words(self, text: str, max_chars: int) -> list:
        """Fallback splitter that respects commas/dashes before breaking on plain words."""
        segments: list[str] = []

        # Prefer to split at punctuation pauses first
        pause_splits = re.split(r'(,|;|:|—|–)', text)
        buffer = ""
        for part in pause_splits:
            buffer += part
            if len(buffer.strip()) >= max_chars:
                segments.extend(self._chunk_by_words(buffer.strip(), max_chars))
                buffer = ""
        if buffer.strip():
            segments.extend(self._chunk_by_words(buffer.strip(), max_chars))
        return segments or [text]

    def _chunk_by_words(self, text: str, max_chars: int) -> list:
        chunks: list[str] = []
        current = ""
    ...
    def _split_sentences(self, text: str) -> list:
        """Tokenize text into sentences using simple punctuation heuristics without regex lookbehind."""
        sentences: list[str] = []
        start = 0
        i = 0
        length = len(text)
        ...
        while i < length:
            ...
            if ch in '.!?':
                ...
                while end < length and text[end] in trailing_closers:
                    end += 1
                ...
                if j >= length or next_char.isupper() or next_char.isdigit() or next_char in "\"'(":
