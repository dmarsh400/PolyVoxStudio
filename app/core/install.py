...
def download_spacy_model():
    """Download required spaCy model (prefer 'md', fallback to 'sm')."""
    print("\n📚 Downloading spaCy language model...")
    # First try the better medium model
    success_md, out_md = run_command([sys.executable, '-m', 'spacy', 'download', 'en_core_web_md'])
    if success_md:
        print("✅ spaCy 'en_core_web_md' downloaded!")
        return True

    # Fallback to small if md fails (offline or version mismatch)
    print("⚠️  Could not download 'en_core_web_md'. Falling back to 'en_core_web_sm'...")
    success_sm, out_sm = run_command([sys.executable, '-m', 'spacy', 'download', 'en_core_web_sm'])
    if success_sm:
        print("✅ spaCy 'en_core_web_sm' downloaded (fallback).")
        print("   Tip: For better accuracy, later run:  python -m spacy download en_core_web_md")
        return True

    print("❌ spaCy model download failed. Try manually:")
