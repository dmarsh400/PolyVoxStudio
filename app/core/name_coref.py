...
class NameCoref:
	...
	def get_variants(self, parts):
		# HYAPH_SPLIT_PATCH: split hyphens/apostrophes into sub-tokens for robust variants
		_parts = []
		for _p in parts:
			_parts.extend(re.split(r"[\-’'`]+", _p))
		parts = [t for t in _parts if t]
				...
				for canonical in canonicals:
					...
					if canonical_name in name_subpart_index:
						for entity in name_subpart_index[canonical_name]:
							...
							# SURNAME_RECENCY_PATCH: prefer linking single-token surname mentions to prior multi-token canonicals
							try:
								if isinstance(entities[i], (list, tuple)) and len(entities[i]) == 1:
									_mention_tok = str(entities[i][0]).lower()
									_last_tok = str(entity).split()[-1].lower()
									if _mention_tok == _last_tok:
										score += 5
										if entity in lastSeen:
											score += 10
							except Exception:
