...
[![GPU Support](https://img.shields.io/badge/GPU-CUDA%2012.1%20%7C%2011.8%20%7C%20CPU-green)](GPU_COMPATIBILITY.md)
...
- ⚙️ GPU acceleration with selectable **CUDA 12.1 / CUDA 11.8 / CPU** modes  
...
Clone the repo and run the platform installer. Each script creates a `PolyVox` virtual environment, installs the right PyTorch wheel (CUDA 11.8, CUDA 12.1, or CPU), and pulls the remaining dependencies.
...
**Driver baseline (NVIDIA):** Ensure your driver supports the selected CUDA version (≥ 520 for CUDA 11.8, ≥ 535 for CUDA 12.1). If in doubt, choose CPU to finish the install, then upgrade drivers and rerun the installer later.
